...
def view_darbuotojai(session):
    try:
        ...
        headers = ["ID", "Vardas", "Pavardė", "Gimimo data", "Atlyginimas", "Pareigų ID", "Darbovietės ID", "Dirba nuo"]
...
def prideti_darboviete(session):

    try:
        # Patikrinam, ar yra darboviečių
        darbovietes = session.query(Darboviete).all()
        if not darbovietes:
            print("Nėra darboviečių. Pirmiausia pridėkite bent vieną darbovietę.")
            return
        
        # Rodo esamas darbovietes
        print("\nGalimos darbovietės:")
        for d in darbovietes:
            print(f"{d.id}: {d.pavadinimas}, {d.miestas}")
        ...
        miestas = input("Įveskite darbovietės lokaciją (miestą): ").strip() or None
        ...
        session.commit()
        print(f"[{darboviete.id}] ID. | {darboviete.pavadinimas}.")
        print("Darbovietė pridėta.")
...
def prideti_pareigas(session):

    try:
        # Patikrinam, ar yra pareigų
        pareigos_list = session.query(Pareigos).all()
        if not pareigos_list:
            print("Nėra pareigų. Pirmiausia pridėkite bent vienas pareigas.")
            return
        
        # Rodo esamas pareigas
        print("\nGalimos pareigos:")
        for p in pareigos_list:
            print(f"{p.id}: {p.pavadinimas} - {p.aprasymas or 'Nėra aprašymo'}")
        ...
        session.commit()
        print(f"[{pareigos.id}] ID. | {pareigos.pavadinimas}.")
        print("Naujos pareigos pridėtos.")
